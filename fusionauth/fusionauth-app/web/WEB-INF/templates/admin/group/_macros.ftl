  ...
  <table class="hover">
    ...
    <tbody>
      [#list groups![] as group]
        <tr>
          <td>${group.name}</td>
          <td class="hide-on-mobile">${group.id}</td>
          [#if tenants?size > 1]
            <td class="hide-on-mobile"> ${helpers.tenantName(group.tenantId)}</td>
          [/#if]
          <td class="action">
            [@button.action href="/admin/group/edit?groupId=${group.id}&tenantId=${group.tenantId}" icon="edit" key="edit" color="blue"/]
...
[#macro groupFields action]
  <fieldset>
    ...
    [/#if]
    [@control.text required=true name="group.name" autofocus="autofocus" tooltip=message.inline('{tooltip}group.name')/]
