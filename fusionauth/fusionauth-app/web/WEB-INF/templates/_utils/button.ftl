...
[#macro formIcon icon="arrow-right" color="blue" textKey="submit" disabled=false autofocus=false]
<button class="${color} button${disabled?then(' disabled', '')}"[#if disabled] disabled="disabled"[/#if][#if autofocus] autofocus="autofocus"[/#if]><i class="fa fa-${icon}"></i> [@message.print key=textKey/]</button>
[/#macro]
